...
$termsUrl = rtrim(config('plugin', 'url_loja', home_url('/')), '/');
$termsUrl = $termsUrl ? $termsUrl . '/termos-de-uso/' : '#';
?>
			...
			<form action="" method="post" class="mp-login-form mp-form">
				...
				<div class="mp-config-content active" data-configuration-step="1">
					...
					<div class="mp-form-row cnpj-fields" style="display: none;">
						...
						<div class="mp-form-col-3">
							<div class="mp-form-group">
								<label class="mp-label"><?= $data['formulario_ramo_atividade'] ?></label>
								<select name="ramo-atividade" class="mp-select" id="ramo-atividade">
									<option value="" selected>Selecione</option>
									<?php foreach ($data['formulario_ramos_atividades_valores'] as $area) { ?>
										<?php $selected = (isset($this->data['fields']['ramo-atividade']) && $this->data['fields']['ramo-atividade'] == $area['business_sectors_id']) ? 'selected="selected"' : ''; ?>
										<option value="<?= $area['business_sectors_id'] ?>" <?= $selected ?>><?= $area['business_sectors_description'] ?></option>
									<?php } ?>
								</select>
							</div>
						</div>
				...
				<div class="mp-config-content active" data-configuration-step="6">
					...
					<div class="mp-checkbox-group receber-ofertas">
						<label class="mp-checkbox">
							...
							<div class="mp-checkbox-label" style="text-align: justify; padding: 0 15px; flex-wrap: wrap; ">Ao me cadastrar, eu confirmo que li e concordo com os <a href="<?= esc_url($termsUrl) ?>" title="Link para a página de detalhes do termo de uso"><b>Termos de Uso</b></a>, Privacidade e Garantia da Ampliz e que receberei notificações, orientações e promoções através dos canais de contato. Podendo desabilitar essa função a qualquer momento. </span></div>
